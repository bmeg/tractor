...
from tractor.dag_helpers import on_failure_callback, pre_execute, on_success_callback, make_tmp_dir
...
class AggregateLoadDagGenerator(BaseDAGGenerator):
    ...
    def mkdags(self, project: Project) -> list[DAG]:
        ...
        assert any([project.defaults.extract_commands, project.defaults.transform_commands, project.defaults.load_commands]), f"Expected at least one command in defaults: {project.defaults}"
        dags = []
        processed_datasets = []
        loaded_dataset = Dataset(f"{project.id}-loaded")
        for source in project.sources:
            raw_dataset = Dataset(f"{source.id}-raw")
            raw_dataset.extra = {'source': source.as_dict()}
            processed_dataset = Dataset(f"{source.id}-processed")
            processed_dataset.extra = {'source': source.as_dict()}
    ...
    def ensure_operator(self, operator_parms: dict, executors: list[tes.Executor] = None, source: tes.Task = None) -> list[BaseOperator]:
        ...
        for i, executor in enumerate(executors):
            ...
            is_tes = executor.image not in ["airflow-operator://BashOperator", "airflow-operator://PythonOperator"]

            # add callback on all
            _operator_parms["on_failure_callback"] = on_failure_callback
            ...
            if i != 0:
                ...
                if not is_tes:
                    # add setup if it is first executor
                    _operator_parms["pre_execute"] = pre_execute

            if i != len(executors) - 1:
                _operator_parms.pop("outlets", None)
            else:
                if not is_tes:
                    # add teardown if it is last executor
                    _operator_parms["on_success_callback"] = on_success_callback
