...
def check_md5(
    response: Any, dag_id: str = None, task_instance: SimpleTaskInstance = None
) -> bool:
    ...
    try:
        ...
        if not new_md5:
            logging.info(
                f"No MD5 found in response from {response.url} reading the content and calculating the MD5"
            )
...
def check_etag(
    response: Any, task_instance: SimpleTaskInstance, header_name: str = "ETag"
) -> bool:
    ...
    try:
        ...
        if not new_etag:
            logging.info(
                f"No ETag found in response from {response.url} -> Triggering the DAG"
            )
        ...
        if stored_etag != new_etag:
            ...
            Variable.set(
                variable_name, new_etag, description=f"ETag for {response.url}"
            )
            return True  # Trigger the DAG
...
def check_last_modified(
    url: str, variable_name: str, timeout: int = 60, header_name: str = "ETag"
) -> bool:
    ...
    try:
        ...
        if stored_value:
            ...
            response = requests.head(
                url, timeout=timeout, headers={"If-Modified-Since": stored_value}
            )
        ...
        if not new_value:
            logging.info(
                f"No last-modified found in response from {url} -> Triggering the DAG"
            )
            return True
        ...
        if stored_value != new_value:
            ...
            Variable.set(
                variable_name, new_value, description=f"last-modified for {url}"
            )
            return True
