...
class Config(BaseModel):
    ...
    def check_config(self):
        ...
        if not self.response_check:
            self.response_check = ResponseCheck(relative_url=next(iter(self.inputs.values())))
        if not self.response_check.checker:
            ...
            if self.response_check.checker is None:
                raise ValidationError(f"Invalid response check type: {self.response_check.type}")

            self.response_check.method = "GET" if self.response_check.type == "md5" else "HEAD"

        self.tags = self.tags + [self.dag_id, "dynamic"]
        self.description = f"{self.description} (Generated: {datetime.now().isoformat()})"
        return self
    ...
    with DAG(**dag_properties) as dag:
        ...
        def validate_task(transform_result: Any, outlet_events) -> Any:
            ...
            for _ in transform_result:
                assert _.endswith('.html'), f"Invalid file extension: {transform_result}"
            validated_results = [_ for _ in transform_result]
            outlet_events[config.processed_dataset].extra = {'manifest': validated_results}
...
def generate_dags_from_config(config_path: str) -> list[DAG]:
    """Generates a DAG from a YAML configuration file."""
    config = Config(**read_config(config_path))
    return [generate_extract_dag(config), generate_transform_dag(config), generate_load_dag(config)]


def generate(config_file_path: str = None):
    """Generates a set of DAG from a YAML configuration file."""
    if config_file_path is None:
        # Example usage: Generate DAG if config file exists
        PROJECT_DIR = Variable.get("project_dir")
        logging.info(f"{__name__}  {PROJECT_DIR}")
        config_file_path = pathlib.Path(PROJECT_DIR) / "config.yaml"
    if os.path.exists(config_file_path):
        globals()["dynamic_dag"] = generate_dags_from_config(str(config_file_path))
        return globals()["dynamic_dag"]
    else:
        raise FileNotFoundError(f"Config file not found: {config_file_path}")


configs = pathlib.Path("/opt/airflow/projects")
for config in configs.glob("stapi-*.yaml"):
    generate(str(config))

# generate("/opt/airflow/projects/config.yaml")
