...
def download_to_s3(
    ...
    logging.info(
        f"Downloading from http_conn_id:{http_conn_id} {relative_url} to s3://{bucket_name}/{s3_prefix}/{file_name} using {aws_conn_id}"
    )
    ...
    if response.status_code == 200:
        ...
        s3_hook.load_bytes(
            response.content, key=s3_key, bucket_name=bucket_name, replace=True
        )
