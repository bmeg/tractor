...
class AggregateLoadDagGenerator(BaseModel):
    ...
    def create_dag(self, dag_id: str, task_id: str, tags: list[str], inlets: list[Dataset], outlets: list[Dataset], task: Task) -> DAG:
    ...
    def ensure_operator(self, operator_parms: dict, task: Task) -> BaseOperator:
        ...
        assert isinstance(task.command, CommandDetailed), f"Expected CommandDetailed got {task.command} see project.apply_defaults()"

        # add callbacks on all
        if task.callbacks:
            if task.callbacks.get("pre_execute"):
                _operator_parms["pre_execute"] = ensure_python_callable(task.callbacks.get("pre_execute"))
            if task.callbacks.get("on_success"):
                _operator_parms["on_success_callback"] = ensure_python_callable(task.callbacks.get("on_success"))
            if task.callbacks.get("on_failure"):
                _operator_parms["on_failure_callback"] = ensure_python_callable(task.callbacks.get("on_failure"))

        if task.command.operator_type == "BashOperator":
            _operator_parms["cwd"] = make_tmp_dir(original_task_id)
            operator = BashOperator(**_operator_parms, bash_command=task.command.command)
        elif task.command.operator_type == "PythonOperator":
            operator = PythonOperator(**_operator_parms, python_callable=ensure_python_callable(task.command.command))
        else:
